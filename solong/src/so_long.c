...
void imgcevir(t_game *game)
{
    ...
    game->img.water = mlx_xpm_file_to_image(game->mlx, "./img/water.xpm", &x, &y);
    game->img.worm = mlx_xpm_file_to_image(game->mlx, "./img/worm.xpm", &x, &y);
	...
	game->img.exit = mlx_xpm_file_to_image(game->mlx, "./img/shell.xpm", &x, &y);
}
...
int main(int argc, char **argv)
{
	...
	if (!check_map_name(argv[1]))
		return (invalid_map());
	game.rowcount = (row_count(game.allmap) + 1) * 64;
	game.columncount = ft_strlen(game.map[0]) * 64;
	if (!checkmap(&game))
		return 0;
	game.mlx = mlx_init();
	game.mlx_win = mlx_new_window(game.mlx, game.columncount, game.rowcount, "BALIK");
	imgcevir(&game);
	maprender(&game, 0);
