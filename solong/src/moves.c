...
int move(t_game *game, int key)
{
	...
	if(key == 0 && game->map[game->prowplace][game->pcolumnplace - 1] != '1')
		s += key_a(game, a, i);
	else if(key == 1 && game->map[game->prowplace +1][game->pcolumnplace] != '1')
		s += key_s(game, a, i);
	else if(key == 2 && game->map[game->prowplace][game->pcolumnplace + 1] != '1')
		s += key_d(game, a, i);
	else if(key == 13 && game->map[game->prowplace - 1][game->pcolumnplace] != '1')
		s += key_w(game, a, i);
	else if(key == 53)
		close_frame(game);
	return (s);
